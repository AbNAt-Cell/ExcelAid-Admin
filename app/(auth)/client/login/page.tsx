...
export default function ClientLogin() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <Link href="/">
        <Image className="mb-8" src="/images/logo.png" alt="Theralink logo" width={180} height={38} priority />
      </Link>
      ...
      <Card className="w-full max-w-md shadow-xl rounded-md border-none">
        <CardHeader>
          <p className="font-bold text-lg inline-flex items-center">
            <Lock className="mr-1" /> Log into Client Portal
          </p>
        </CardHeader>
        <CardContent>
          {error && (
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                    ...
                    <div className="space-y-1 leading-none">
                      <FormLabel>Login as guardian</FormLabel>
                    </div>
                  </FormItem>
                )}
              />
              <Button variant="secondary" type="submit" className="w-full" disabled={form.formState.isSubmitting}>
                {form.formState.isSubmitting ? (
                  ...
                  <span className="font-bold">
                    Login <ArrowRight className="inline-block ml-1" />
                  </span>
                )}
              </Button>
            </form>
